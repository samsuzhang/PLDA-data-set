...
#include <vector>
#include <sstream>
#include "data_analysis_case__illegal_6__masked_cmp_leak.h"
...
int main(int argc, char const* argv[])
{
	...
	while (bufiss.getline(line, 10000)) {
		...
		while (iss >> tmp_str) {
			// tmp_vec.push_back(implicit_process_by_branch(tmp_str));
	...
	string result = illegal_analyse_6(input1, input2);
	//printf("result: %s\n", result);
	return 0;
}